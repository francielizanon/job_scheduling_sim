...
class FCFSTest(unittest.TestCase):
    def test_10000_nodes(self):
        simulator = Engine('fcfs', 10000, -1, '../ANL-Intrepid-2009-1.swf')
        makespan = simulator.run()
        self.assertEqual(makespan, 38119656)

    def test_10000_tasks(self):
        simulator = Engine('fcfs', 40960, 10000, '../ANL-Intrepid-2009-1.swf')
        makespan = simulator.run()
        self.assertEqual(makespan, 3610649)

    def test_10000_tasks_and_nodes(self):
        simulator = Engine('fcfs', 10000, 10000, '../ANL-Intrepid-2009-1.swf')
        makespan = simulator.run()
        self.assertEqual(makespan, 5704150)
